...
export const CSharpType = ({ type, links }: { type: string, links: string[] }) => {
    ...
    const colorizeType = (text: string, links: string[]): React.ReactNode => {
        ...
        const primitives = ["string", "int", "bool", "float", "double", "decimal", "char", "byte", "object", "void", "long", "short", "uint", "ulong"];
        
        // Common .NET types (green)
        const commonTypes = ["Guid", "DateTime", "TimeSpan", "Uri", "Exception", "Task", "List", "Dictionary", "IEnumerable", "ICollection", "HashSet", "Array"];
                ...
                <>
                    {parts.map((part, index) => (
                        <span key={index}>
                            ...
                            {index < parts.length - 1 && <span style={{ color: colors.punctuation }}>, </span>}
                        </span>
                    ))}
                </>
            );
        ...
        if (primitives.includes(text.toLowerCase())) {
            return links && links.length > 0 && links[0] ? (<a href={links[0]} target={links[0].startsWith('http') ? '_blank' : '_self'} className="no-underline text-inherit hover:text-inherit"><span style={{ color: colors.primitive }}>{text}</span></a>) : <span style={{ color: colors.primitive }}>{text}</span>;
        }
        ...
        return links && links.length > 0 && links[0] ? (<a href={links[0]} target={links[0].startsWith('http') ? '_blank' : '_self'} className="no-underline text-inherit hover:text-inherit"><span style={{ color: colors.custom }}>{text}</span></a>) : <span style={{ color: colors.custom }}>{text}</span>;
    };
